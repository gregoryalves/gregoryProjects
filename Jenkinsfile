pipeline {
 ...
 stages {
	stage('Build Imagem Docker! ') {
		steps {
			sh 'cp /var/lib/jenkins/workspace/trabalho-sidnei_master/dist/Lanchonete.war /home/lucasjansen/docker/'
	...
	stage('Executar') {
		steps {
			sh 'docker container create --publish 8081:8080 trabalho-sidnei-'
	...
	stage('Remover Workspace') {
		steps {
			...
			sh 'rm -f /home/lucasjansen/docker/Lanchonete.war'
		}
	}
 }
}